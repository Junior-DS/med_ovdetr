...
def get_args_parser():
    parser = argparse.ArgumentParser("Medical OV DETR Detector", add_help=False)
    parser.add_argument("--lr", default=1e-4, type=float,
                        help = "Base learning rate") # changed to 1e-4 for medical images
    ...
    parser.add_argument("--batch_size", default=4, type=int,
                        help = "Batch size (adjust for GPU memory)") # changed to 4 to fits the 512x512 images
    ...
    # Add new parameter
    parser.add_argument("--num_lesion_classes", default=11, type=int,
                   help="Number of lesion categories (11 for your dataset)")
    ...
    parser.add_argument("--dataset_file", default="cytology",
                    help="Identifier for medical dataset")
    parser.add_argument("--cytology_path", default="OV-DETR/ovdetr/cytology", type=str,
                    help="Root path to cytology dataset")
    parser.add_argument("--medical_embed_path", default="OV-DETR/ovdetr/cytology/json_files/medical_embeddings.json", type=str,
                   help="Path to clinical text embeddings")
...
def main(args):
    ...
    # added this line of codes
    # Loads your clinical text embeddings
    # Attaches them to the model for conditional matching
    # Provides class name reference
    # After line: model.to(device)
    if args.dataset_file == "cytology":
        # Load clinical embeddings
        with open(args.medical_embed_path) as f:
            medical_embeddings = json.load(f)
        
        # Create embeddings directory if missing
        Path("embeddings").mkdir(exist_ok=True)
        
        # Get device-appropriate model reference
        model_ref = model.module if args.distributed else model
        
        # Load or generate CLIP features
        if not Path("embeddings/ascus.npy").exists():
            print("Generating CLIP embeddings...")
            from clip import clip
            clip_model, _ = clip.load("ViT-B/32", device=device)
            
            with torch.no_grad():
                for cls, desc in medical_embeddings.items():
                    text = clip.tokenize([desc]).to(device)
                    features = clip_model.encode_text(text).cpu().numpy()
                    np.save(f"embeddings/{cls}.npy", features)
        
        # Load embeddings
        text_features = [
            torch.from_numpy(np.load(f"embeddings/{cls}.npy"))
            for cls in medical_embeddings.keys()
        ]
        model_ref.medical_text_embeddings = torch.stack(text_features).squeeze(1).to(device)
        model_ref.lesion_classes = list(medical_embeddings.keys())
    ...
    # Medical class configuration
    if args.dataset_file == "cytology":
        # Set model dimensions
        model.num_classes = len(dataset_train.CLASSES)
        
        # Verify class count matches embeddings
        assert model.num_classes == args.num_lesion_classes, \
            f"Configured {args.num_lesion_classes} classes but found {model.num_classes}"
        
        print(f"Initialized for medical detection with {model.num_classes} classes:")
        print(", ".join(dataset_train.CLASSES))
    ...
    if args.dataset_file == "cytology":
        required_paths = [
            args.cytology_path,
            args.medical_embed_path,
            f"{args.cytology_path}/updated_train",
            f"{args.cytology_path}/splits/base_train.json",
            f"{args.cytology_path}/splits/base_val.json"
        ]
        for p in required_paths:
            if not Path(p).exists():
                raise FileNotFoundError(f"Medical dataset path missing: {p}")
    ...
    if args.dataset_file == "coco_panoptic":
        ...
        coco_val = data.coco.build("val", args)
    ...
    if args.eval:
        if args.dataset_file == "cytology":
            from engine_medical import medical_evaluate  # Custom medical evaluator
            test_stats = medical_evaluate(
    ...
    if args.label_map:
        ...
        if args.masks:
            for key, value in model.module.detr.clip_feat.items():
                new_clip_feat[data_loader_train.dataset.cat2label[key]] = value
